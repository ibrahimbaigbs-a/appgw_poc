...
variable "public_ip_address_configuration" {
  ...
  default     = {}
  description = <<-DESCRIPTION
  - `resource_group_name`: (Optional) The name of the resource group to create the public IP address in. If not specified, will use the application gateway resource group.
  - `location`: (Optional) The location to create the public IP address in. If not specified, will use the application gateway location.
  ...
  - `zones`: (Optional) A collection containing the availability zone to allocate the Public IP in. Changing this forces a new resource to be created. Availability Zones are only supported with a Standard SKU and in select regions at this time.
...
variable "rewrite_rule_set" {
 ...
 - `variable` - (Required) The [variable](https://docs.microsoft.com/azure/application-gateway/rewrite-http-headers-url#server-variables) of the condition.
...
variable "ssl_profile" {
 ...
 - `verify_client_certificate_revocation` - (Optional) Verify client certificate revocation status.
